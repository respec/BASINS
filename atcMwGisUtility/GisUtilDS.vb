...
Public Class GisUtilDS
    ...
    <CLSCompliant(False)>
    Public Shared WriteOnly Property MappingObject() As AppManager
        Set(ByVal aNewValue As AppManager)
            pMapWin = aNewValue
        End Set
    End Property
    ...
    ''' <summary>Map Window Object</summary>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property GetMappingObject() As AppManager
        Get
            If pMapWin Is Nothing Then
                'Throw New MappingObjectNotSetException
                Throw New ApplicationException("Map control is not present.")
            Else
                Return pMapWin
            End If
        End Get
    ...
    Private Shared Function MapLayers() As ArrayList
        ...
        Dim ptlayers As List(Of IMapFeatureLayer) = GetFeatureLayers(FeatureType.Point)
        Dim linelayers As List(Of IMapFeatureLayer) = GetFeatureLayers(FeatureType.Line)
        Dim polylayers As List(Of IMapFeatureLayer) = GetFeatureLayers(FeatureType.Polygon)
        lMapLayers.AddRange(ptlayers)
        lMapLayers.AddRange(linelayers)
        lMapLayers.AddRange(polylayers)
        Return lMapLayers
    End Function
        ...
        If Not pMapWin Is Nothing AndAlso Not pMapWin.Map.Layers Is Nothing Then
            For Each layer As ILayer In pMapWin.Map.Layers
                ...
                If lMG IsNot Nothing Then
                    ...
                    Next
                Else
                    Dim lFlyr As IFeatureLayer = TryCast(layer, IFeatureLayer)
                    If lFlyr IsNot Nothing Then
                        If lScreenGeometryType Then
                            If lFlyr.DataSet.FeatureType = fType Then
                                If lScreenLayerName Then
                                    If lFlyr.LegendText.ToLower() = aLayerName.ToLower() OrElse lFlyr.DataSet.Name.ToLower() = aLayerName.ToLower() Then
    ...
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared Function FeatureIndexValid(ByVal aFeatureIndex As Integer, ByVal aSf As Shapefile) As Boolean
        Dim lNumShapes As Integer = aSf.Features.Count
        If aFeatureIndex < 0 Or aFeatureIndex >= lNumShapes Then
            Throw New Exception("GisUtil:FieldValue:Error:FeatureIndex:" & aFeatureIndex & ":OutOfRange:0:" & lNumShapes - 1)
        Else
            Return True
        End If
    End Function
    ...
    Public Shared Sub LoadProject(ByVal aProjectName As String)
        If FileExists(aProjectName) Then
            ...
            Dim lRet As Boolean
            Try
                GetMappingObject.SerializationManager.OpenProject(aProjectName)
                lRet = True
    ...
    ''' <summary>
    ''' ToDo: this doesn't seems right, Layers are actually layer groups in the legend
    ''' </summary>
    ''' <returns></returns>
    Public Shared Function CurrentLayer() As IMapLayer
        Try
    ...
    Public Shared Function NumSelectedFeatures(ByVal aLayer As IMapFeatureLayer) As Integer
        If aLayer IsNot Nothing Then
            Return aLayer.Selection.Count
        Else
            Return -1
        End If
    End Function
    ...
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    ''' <remarks>DotSpatial version</remarks>
    <CLSCompliant(False)>
    Public Shared ReadOnly Property LayerType(ByVal aLayerName As String) As FeatureType
        Get
            Dim listLayers As List(Of IMapFeatureLayer) = GetFeatureLayers(Nothing, aLayerName)
            For Each lyr As IMapFeatureLayer In listLayers
                Return lyr.DataSet.FeatureType
            Next
            Return Nothing
        End Get
    End Property
    ...
    Public Shared ReadOnly Property ProjectFileName() As String
        Get
            Return GetMappingObject.SerializationManager.CurrentProjectFile
        End Get
    End Property

    ''' <summary>
    ''' Find features in aFS1 that intersect features in aFS2 (such as a selection polygon)
    ''' The looping might make it quite slow if inputs are large...
    ''' Source: https://stackoverflow.com/questions/19219732/intersection-between-two-geographic-lines
    ''' </summary>
    ''' <param name="aFS1"></param>
    ''' <param name="aFS2"></param>
    ''' <returns>Return features in aFS1 that intersect features in aFS2</returns>
    Public Shared Function GP_Intersect(ByVal aFS1 As IFeatureSet, ByVal aFS2 As IFeatureSet) As IFeatureSet
        Dim ft As FeatureType = aFS1.FeatureType
        Dim result As IFeatureSet = New FeatureSet(ft)
        result.Projection = aFS1.Projection
        For Each lf1 As IFeature In aFS1.Features
            For Each lf2 As IFeature In aFS2.Features
                If lf1.Intersects(lf2) Then
                    result.AddFeature(lf1.Geometry)
                    Exit For 'touch any one in aFS2 is enough
                End If
            Next
        Next
        Return result
    ...
    ''' </summary>
    ''' <param name="aInputLayerName"></param>
    ''' <param name="aOutputLayerName"></param>
    ...
    Public Shared Sub SaveSelectedFeatures(ByVal aInputLayerName As String, ByVal aOutputLayerName As String,
        ...
        Select Case lSourceLayer.DataSet.FeatureType
            Case FeatureType.Line
                lOutLayer = New LineShapefile
            Case FeatureType.Point
                lOutLayer = New PointShapefile
            Case FeatureType.Polygon
                lOutLayer = New PolygonShapefile
        End Select
        lOutLayer.Projection = lSourceLayer.Projection
        lOutLayer.SaveAs(aOutputLayerName, True)
        If aAddToMap Then
    ...
    Public Shared Function AddLayer(ByVal aFileName As String, ByVal aLayerName As String) As Boolean
        ...
        Else
            ...
            Else
                Try
                    Dim fs As IFeatureSet = FeatureSet.Open(aFileName)
                    Dim mylayer As IMapFeatureLayer = pMapWin.Map.Layers.Add(fs)
    ...
    ''' </param>
    Public Shared Function IsLayerByFileName(ByVal aLayerFileName As String, Optional ByRef aLayer As IMapFeatureLayer = Nothing) As Boolean
        Dim layers As List(Of IMapFeatureLayer) = GetFeatureLayers(Nothing)
        For Each layer As IMapFeatureLayer In layers
            If layer.DataSet.Filename = aLayerFileName Then
                aLayer = layer
                Return True
            End If
        Next
    ...
    Public Shared Function GetLayerByFileName(ByVal aLayerFileName As String) As IMapFeatureLayer
        Dim layer As IMapFeatureLayer = Nothing
        IsLayerByFileName(aLayerFileName, layer)
        Return layer
    End Function
    ...
    ''' </param>
    Public Shared Function IsLayer(ByVal aLayerName As String, Optional ByRef aLayer As IMapFeatureLayer = Nothing) As Boolean
        Dim layers As List(Of IMapFeatureLayer) = GetFeatureLayers(Nothing, aLayerName)
    ...
    Public Shared Function GetLayerByName(ByVal aLayerName As String) As IMapFeatureLayer
        Dim layer As IMapFeatureLayer = Nothing
        IsLayer(aLayerName, layer)
        Return layer
    End Function

    Public Shared Sub ZoomToLayer(ByVal aLayer As IMapFeatureLayer)
        pMapWin.Map.ViewExtents = aLayer.DataSet.Extent
        pMapWin.Map.Refresh()
    End Sub

    Public Shared Sub CreateShapefileOfCurrentMapExtents(ByVal aShapeName As String)
        Try
            Dim lExtentsFS As IFeatureSet = New FeatureSet()
            lExtentsFS.FeatureType = FeatureType.Polygon
            lExtentsFS.Extent = pMapWin.Map.Extent
            lExtentsFS.SaveAs(aShapeName, True)
        Catch ex As Exception
            Logger.Dbg("Failed to create shape of extents.")
        End Try
    End Sub

    Public Shared Function ProjectLayer(ByVal aLayer As IMapFeatureLayer, ByVal aNewProj As DotSpatial.Projections.ProjectionInfo) As Boolean

    End Function

    Public Shared Function GetFieldIndexByName(ByVal aLayer As IMapFeatureLayer, ByVal aHeaderName As String) As Integer
        If aLayer Is Nothing Then Return -1
        If String.IsNullOrEmpty(aHeaderName) Then Return -1

        Dim lDataTable As DataTable = aLayer.DataSet.DataTable
        Dim lFieldIndex As Integer = -1
        For lCol As Integer = 0 To lDataTable.Columns.Count - 1
            If lDataTable.Columns.Item(lCol).ColumnName.ToLower = aHeaderName.ToLower Then
                lFieldIndex = lCol
                Exit For
            End If
        Next
    ...
    Public Shared Function GetTableValuesAtColumn(ByVal aLayer As IMapFeatureLayer,
                                                  ByVal aColumnName As String,
                                                  Optional ByVal aColumnIndex As Integer = -1,
                                                  Optional ByVal aSelected As Boolean = True) As List(Of String)

        If aLayer Is Nothing Then Return Nothing
        If String.IsNullOrEmpty(aColumnName) AndAlso aColumnIndex < 0 Then Return Nothing
        Dim lColumnIndex As Integer = aColumnIndex
        If aColumnIndex < 0 Then
            lColumnIndex = GetFieldIndexByName(aLayer, aColumnName)
        End If
        If lColumnIndex < 0 Then Return Nothing
        Dim lDataTable As DataTable = aLayer.DataSet.DataTable
        Dim lNumRows As Integer = aLayer.DataSet.NumRows()

        Dim lAttributeValues As New List(Of String)
        If aSelected Then
            For Each lFeat As IFeature In aLayer.Selection.ToFeatureList()
                lAttributeValues.Add(lFeat.DataRow.Item(lColumnIndex).ToString())
            Next
        Else
            For lRow As Integer = 0 To lNumRows - 1
                Dim lFeat As IFeature = aLayer.DataSet.GetFeature(lRow)
                lAttributeValues.Add(lFeat.DataRow.Item(lColumnIndex).ToString())
            Next
        End If
    ...
    Public Shared Function GetSelectedExtentInLayer(ByVal aLayer As IMapFeatureLayer) As Extent
        If aLayer Is Nothing Then Return Nothing
        Dim lSelection As ISelection = aLayer.Selection
        If lSelection.Count = 0 Then
            Return Nothing
        Else
            Return lSelection.ToFeatureSet.Extent
        End If
    End Function

    Public Shared Function GetSelectedExtent(ByVal aLayers As List(Of IMapFeatureLayer)) As Extent
        If aLayers Is Nothing OrElse aLayers.Count = 0 Then
            Return Nothing
        End If
        Dim lExt As New Extent()
        For Each lyr As IMapFeatureLayer In aLayers
            Dim lExtlyr As Extent = GetSelectedExtentInLayer(lyr)
            If lExtlyr IsNot Nothing Then
                lExt.ExpandToInclude(lExtlyr)
            End If
        Next
        Return lExt
    End Function

    Public Shared Function SelectByShape(ByVal aSelectLayer As IMapFeatureLayer, ByVal aSelectByLayer As IMapFeatureLayer) As List(Of IFeature)
        Dim lSelection As ISelection = aSelectByLayer.Selection
        If lSelection.Count = 0 Then Return Nothing
        Dim aoi As Extent = lSelection.ToFeatureSet.Extent
        Dim lSelectedFeatures As List(Of IFeature) = aSelectLayer.DataSet.Select(aoi)
        Return lSelectedFeatures
    End Function
    ...
    Public Shared ReadOnly Property LayerFileName(ByVal aLayerName As String) As String
        Get
            Dim layer As IMapFeatureLayer = GetLayerByName(aLayerName)
...
#Else
    ...
    Public Shared Sub ComputeToWetlands(ByVal aFlowDirGridFileName As String, ByVal aWetlandsFileName As String, ByVal aStreamsFileName As String, ByVal aToWetlandsGridFileName As String)
        ...
        With lStreamsGrid
            .Open(lTmpStreamGridFileName)
            Dim lNumRows As Integer = .Header.NumberRows
            Dim lNumCols As Integer = .Header.NumberCols
            For lSRow As Integer = 0 To lNumRows - 1
                For lSCol As Integer = 0 To lNumCols - 1
                    If .Value(lSCol, lSRow) > 0 Then
                        'find corresponding cell in the output grid
                        .CellToProj(lSCol, lSRow, lXPos, lYPos)
                        lOutputGrid.ProjToCell(lXPos, lYPos, lCol, lRow)
                        lOutputGrid.Value(lCol, lRow) = 98  'this is a stream
                    End If
                Next
                Logger.Progress(lSRow, lNumRows)
            Next
            .Close()
        End With
        ...
        'store output grid in an array for quicker access
        Dim lArray(lOutputGrid.Header.NumberCols, lOutputGrid.Header.NumberRows) As Integer
        For lRow = 0 To lOutputGrid.Header.NumberRows - 1
            For lCol = 0 To lOutputGrid.Header.NumberCols - 1
                lArray(lCol, lRow) = lOutputGrid.Value(lCol, lRow)
            Next
        Next
        ...
        Do Until lFound = False
            ...
            lFound = MarkCellsDrainingToWetlandsOrStreams(lArray, 99)
        Loop
        Logger.Progress(100, 100)
        'lFound = True
        'Do Until lFound = False
        ...
        For lRow = 0 To lOutputGrid.Header.NumberRows - 1
            For lCol = 0 To lOutputGrid.Header.NumberCols - 1
                lOutputGrid.Value(lCol, lRow) = lArray(lCol, lRow)
            Next
        Next
    ...
    Private Shared Function MarkCellsDrainingToWetlandsOrStreams(ByRef aArray As Integer(,), ByVal aCode As Integer) As Boolean
        'now for each cell of the output grid that is 98 (stream) or 99 (wetland)
        'look for cells that drain to it.  mark those cells as 98/99.
        'then mark this cell as 28/29 to indicate already searched neighbors.
        'stop when nothing has been found to change.
        ...
        Dim lNumRows As Integer = aArray.GetUpperBound(1)
        Dim lNumCols As Integer = aArray.GetUpperBound(0)
        ...
        For lRow As Integer = 0 To lNumRows - 1
            For lCol As Integer = 0 To lNumCols - 1
                If aArray(lCol, lRow) = aCode Then
                    'see if anything around it drains to it
                    If lCol > 0 AndAlso lRow > 0 AndAlso aArray(lCol - 1, lRow - 1) = 8 Then
                        'that cell flows to this one
                        aArray(lCol - 1, lRow - 1) = aCode
                    End If
                    If lRow > 0 AndAlso aArray(lCol, lRow - 1) = 7 Then
                        'that cell flows to this one
                        aArray(lCol, lRow - 1) = aCode
                    End If
                    If lCol < lNumCols - 1 AndAlso lRow > 0 AndAlso aArray(lCol + 1, lRow - 1) = 6 Then
                        'that cell flows to this one
                        aArray(lCol + 1, lRow - 1) = aCode
                    End If
                    If lCol > 0 AndAlso aArray(lCol - 1, lRow) = 1 Then
                        'that cell flows to this one
                        aArray(lCol - 1, lRow) = aCode
                    End If
                    If lCol < lNumCols - 1 AndAlso aArray(lCol + 1, lRow) = 5 Then
                        'that cell flows to this one
                        aArray(lCol + 1, lRow) = aCode
                    End If
                    If lCol > 0 AndAlso lRow < lNumRows - 1 AndAlso aArray(lCol - 1, lRow + 1) = 2 Then
                        'that cell flows to this one
                        aArray(lCol - 1, lRow + 1) = aCode
                    End If
                    If lRow < lNumRows - 1 AndAlso aArray(lCol, lRow + 1) = 3 Then
                        'that cell flows to this one
                        aArray(lCol, lRow + 1) = aCode
                    End If
                    If lCol < lNumCols - 1 AndAlso lRow < lNumRows - 1 AndAlso aArray(lCol + 1, lRow + 1) = 4 Then
                        'that cell flows to this one
                        aArray(lCol + 1, lRow + 1) = aCode
                    End If
                    aArray(lCol, lRow) = aCode - 70  'this is the type we were looking for and it has now been checked
                    lFound = True
                    If lInitRow = 0 Then
                        lInitRow = lRow
                    End If
