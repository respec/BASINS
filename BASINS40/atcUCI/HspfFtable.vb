...
<System.Runtime.InteropServices.ProgId("HspfFtable_NET.HspfFtable")> Public Class HspfFtable
	...
	Dim pId As Integer
	Dim pNrows As Integer
	Dim pNcols As Integer
			...
			With lOpn.FTable
				...
				For i = 1 To .Nrows
					...
					If NumericallyTheSame(.DepthAsRead(i), .Depth(i)) Then
						s = .DepthAsRead(i)
					Else
						s = RSet(CStr(VB6.Format(.Depth(i), fmt)), Len(s))
					End If
					t = Space(10)
					If NumericallyTheSame(.AreaAsRead(i), .Area(i)) Then
						t = .AreaAsRead(i)
					Else
						t = RSet(CStr(VB6.Format(.Area(i), fmt)), Len(t))
					End If
					s = s & t
					t = Space(10)
					If NumericallyTheSame(.VolumeAsRead(i), .Volume(i)) Then
						t = .VolumeAsRead(i)
					Else
						t = RSet(CStr(VB6.Format(.Volume(i), fmt)), Len(t))
					End If
					s = s & t
					For j = 1 To .Ncols - 3
						t = Space(10)
						If j = 1 Then
							If NumericallyTheSame(.Outflow1AsRead(i), .Outflow1(i)) Then
								t = .Outflow1AsRead(i)
							Else
								t = RSet(CStr(VB6.Format(.Outflow1(i), fmt)), Len(t))
							End If
						End If
						If j = 2 Then
							If NumericallyTheSame(.Outflow2AsRead(i), .Outflow2(i)) Then
								t = .Outflow2AsRead(i)
							Else
								t = RSet(CStr(VB6.Format(.Outflow2(i), fmt)), Len(t))
							End If
						End If
						If j = 3 Then
							If NumericallyTheSame(.Outflow3AsRead(i), .Outflow3(i)) Then
								t = .Outflow3AsRead(i)
							Else
								t = RSet(CStr(VB6.Format(.Outflow3(i), fmt)), Len(t))
							End If
						End If
						If j = 4 Then
							If NumericallyTheSame(.Outflow4AsRead(i), .Outflow4(i)) Then
								t = .Outflow4AsRead(i)
							Else
								t = RSet(CStr(VB6.Format(.Outflow4(i), fmt)), Len(t))
							End If
						End If
						If j = 5 Then
							If NumericallyTheSame(.Outflow5AsRead(i), .Outflow5(i)) Then
								t = .Outflow5AsRead(i)
							Else
								t = RSet(CStr(VB6.Format(.Outflow5(i), fmt)), Len(t))
							End If
						End If
						s = s & t
					Next j
	...
	Public Sub FTableFromCrossSect(ByRef dL As Single, ByRef dYm As Single, ByRef dWm As Single, ByRef dN As Single, ByRef dS As Single, ByRef dM11 As Single, ByRef dM12 As Single, ByRef dYc As Single, ByRef dM21 As Single, ByRef dM22 As Single, ByRef dYt1 As Single, ByRef dYt2 As Single, ByRef dM31 As Single, ByRef dM32 As Single, ByRef dW11 As Single, ByRef dW12 As Single)
		...
		If dYm < 0# Or dWm < 0# Or dN < 0# Or dS < 0# Or dM11 < 0# Or dM12 < 0# Or dM21 < 0# Or dM22 < 0# Or dM31 < 0# Or dM32 < 0# Then
			Nrows = 1
			Ncols = 4
			pDepth(1) = 0
			pArea(1) = 0
		...
		Else
			...
			For i = 1 To Nrows
				...
				Else
					dDepth = dYc
				End If
				dDenominator = dDenominator + dDepth * (System.Math.Sqrt(1# + 1# / (dM11 * dM11)) + System.Math.Sqrt(1# + 1# / (dM12 * dM12)))
				If (Depth(i) > dYc) Then
					If dYt1 > Depth(i) Then
						dDepth = Depth(i)
					Else
						dDepth = dYt1
					End If
					dDenominator = dDenominator + dW11 + dW12 + (dDepth - dYc) * (System.Math.Sqrt(1# + 1# / (dM21 * dM21)) + System.Math.Sqrt(1# + 1# / (dM22 * dM22)))
				End If
				If (Depth(i) > dYt1) Then
					If dYt2 > Depth(i) Then
						dDepth = Depth(i)
					Else
						dDepth = dYt2
					End If
					dDenominator = dDenominator + (dDepth - dYt1) * (System.Math.Sqrt(1# + 1# / (dM31 * dM31)) + System.Math.Sqrt(1# + 1# / (dM32 * dM32)))
				End If
