...
Friend Module modSWMMFromMW
    ...
    Public Sub UseDefaultsForNodeAttributes(ByVal aFieldDetails As atcSWMMFieldDetails, ByRef aNodes As Nodes)
        'for any value that didn't come from the attribute table, use user-supplied default
        'For Each lNode As Node In aNodes
        '    For Each lField As System.Reflection.FieldInfo In lNode.GetType.GetFields()
        '        Dim lFieldName As String = lField.Name
        '    Next

        '    'if a name shows up in the aFieldMap, it has been automatically mapped from the dbf, so we don't want to set it to the default 



        '    If Not aFieldMap.Contains("InitDepth") Then
        '        'no default set through field mapping
        '        If aFieldNames.IndexOf("InitDepth") > -1 Then
        '            'we have a use user-supplied default, use it
    ...
    Public Sub UseDefaultsForConduitAttributes(ByVal aFieldDetails As atcSWMMFieldDetails, ByRef aConduits As Conduits)
        'for any value that didn't come from the attribute table, use user-supplied default
    ...
    Public Sub UseDefaultsForCatchmentAttributes(ByVal aFieldDetails As atcSWMMFieldDetails, ByRef aCatchments As Catchments)
        'for any value that didn't come from the attribute table, use user-supplied default
        'For Each lCatchment As Catchment In aCatchments
        '    'todo:  use reflection to go through class properties
        '    'no default set through field mapping, use user-supplied default
