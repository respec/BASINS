...
Public Module FillMissing_HPCP
    ...
    Private Const pMaxPctMiss As Integer = 20
    ...
    Public Sub ScriptMain(ByRef aMapWin As IMapWin)
        Logger.StartToFile(pOutputPath & "Fill_HPCP_Special.log", , , True)
        ...
        For Each lfile As String In lFiles
            ...
            If Not pAlreadyDone.Contains(lStatePath.Substring(0, 2)) Then
                'Logger.StartToFile(pOutputPath & lStatePath & lStation & "_Fill" & ".log", , , True)
                    ...
                    For Each lts As atcTimeseries In lWDMfile.DataSets
                        ...
                        Select Case lCons 'look for wanted constituents and check % missing
                            Case "HPCP" ', "HPCP1", "TMIN", "TMAX", "PRCP"
                                Dim ld(5) As Integer
                                J2Date(lts.Attributes.GetValue("EJDay"), ld)
                                If Double.IsNaN(lts.Values(lts.numValues)) Then
                                    'last value is missing, fill it properly
                                    Logger.StartToFile(pOutputPath & lStatePath & lStation & "_Fill_HPCP" & ".log", , , True)
                                    'lNewWDM = pOutputPath & lStatePath & lStation & ".wdm"
                        ...
                        If lAddMe Then
                                    ...
                                    If FileExists(lFileName) Then
                                        ...
                                        For k As Integer = 0 To lFillTsers.Count - 1
                                            ...
                                            If Not lFillTS Is Nothing Then
                                                ...
                                                'Else
                                                '    Logger.Dbg("FillMissing:  NOT Using station " & lStaFill & _
                                                '               " - no data for time period available")
                                            End If
                                            'Else
                                            '    Logger.Dbg("FillMissing:  No WDM file for station " & lStaFill)
                                        Next
                                    End If
                                    i += 1
                                End While
                            ...
                            End If
                            'write filled data set to existing WDM file
                            Dim lUpdWDMFile As New atcWDM.atcDataSourceWDM
                            lUpdWDMFile.Open(lfile)
                            If lUpdWDMFile.AddDataset(lts, atcDataSource.EnumExistAction.ExistReplace) Then
                                Logger.Dbg("FillMissing:  Updated " & lCons & " dataset on WDM file for station " & lStation)
                            Else
                                Logger.Dbg("FillMissing:  PROBLEM updating " & lCons & " dataset on WDM file for station " & lStation)
                            End If
                            lUpdWDMFile = Nothing
                            Logger.StartToFile(pOutputPath & "Fill_HPCP_Special.log", True, , True)
                        End If
                    Next
