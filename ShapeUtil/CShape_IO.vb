...
Friend Class CShape_IO
	...
	Public Function ShapeFileOpen(ByVal filename As String, ByVal RWFLAG As Integer) As Boolean
		...
		If isFileNameValid(filename) = False Then 'Raise error if something is wrong with shapefile
			Err.Raise(ShapeIOError.InvalidShapeFile, "CShape_IO::ShapefileOpen", "The specified shapefile is not valid or extension is invalid")
			Exit Function
		End If
		...
		sFnR = FreeFile
		FileOpen(sFnR, ShapeFilename, OpenMode.Binary) 'open the shapefile
        FileGet(sFnR, ShapeFileHeader)
		With ShapeFileHeader 'swap bytes of fields stored big endian
			.FileCode = SwapBytes(.FileCode)
			If .FileCode <> shpFileCode Then 'Problem with the file
				Err.Raise(ShapeIOError.InvalidShapeFile, "CShape_IO::ShapefileOpen", "The specified shapefile does not start with a valid code (" & .FileCode & ")")
				Exit Function
			End If
			.FileLength = SwapBytes(.FileLength)
			.u1 = SwapBytes(.u1)
				...
				With Polygon
					...
					Select Case .ShapeType 'MultiPoint types have no parts
						Case FILETYPEENUM.typeMultipoint, FILETYPEENUM.typeMultiPointZ, FILETYPEENUM.typeMultiPointM
							.NumParts = 0
						Case Else
                            FileGet(sFnR, .NumParts)
					End Select
					
                    FileGet(sFnR, .NumPoints)
					...
					If .NumParts > 0 Then
						...
						If .ShapeType = FILETYPEENUM.typeMultiPatch Then
							...
							FileGet(sFnR, .PartTypes)
						End If
					End If
					
					ReDim .thePoints(.NumPoints - 1)
                    FileGet(sFnR, .thePoints)
...
ERR_ROUTINE: 
	...
	Private Sub getRecordHeader(ByRef offset As Integer)
        FileGet(sFnR, RecordHeader, offset)
		With RecordHeader
	...
	Private Function CreateTmpShp(ByRef iTfn As Integer, ByRef sTfn As Integer, ByVal ShapeType As Integer) As Boolean
		...
		sTfn = FreeFile
		FileOpen(sTfn, thePath & shptmp, OpenMode.Binary)
		iTfn = FreeFile
	...
	Friend Sub putXYPoint(ByVal RecordNumber As Integer, ByRef theRecord As ShapeDefines.T_shpXYPoint)
		...
		Else
			...
			indxRecHdr.ContentLength = SwapBytes(recsize / 2)
			indxRecHdr.offset = SwapBytes(shpFileSize / 2)
			FilePut(sFnR, shpRecHdr, shpFileSize + 1)
			FilePut(sFnR, theRecord)
...
ERR_ROUTINE: 
	...
	Friend Sub putPoly(ByVal RecordNumber As Integer, ByRef theRecord As T_shpPoly)
		...
		If RecordNumber <> Append Then
			CreateTmpShp(iTfn, sTfn, ShapeFileHeader.ShapeType)
			shpFileSize = FILESIZES.MainHeaderSize
			indxfilesize = FILESIZES.MainHeaderSize
			For i = 1 To RecordNumber - 1
				IOrecord = getPoly(i)
...
ERR_ROUTINE: 
	...
	Public Function CreateNewShape(ByVal filename As String, ByRef ShapeType As Integer) As Boolean
		...
		FileOpen(sFnR, ShapeFilename, OpenMode.Binary)
		ShapeIsOpen = True
		iFnR = FreeFile
