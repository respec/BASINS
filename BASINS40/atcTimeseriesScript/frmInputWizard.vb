...
Friend Class frmInputWizard
	...
	Public WriteOnly Property TserFile() As ATCclsTserFile
		Set(ByVal Value As ATCclsTserFile)
			pTserFile = Value
		End Set
	End Property
	...
	Private Sub SetRulers()
		...
		RulerString1 = ""
		RulerString2 = ""
		NumChars = VB6.PixelsToTwipsX(txtRuler2.Width) / CharWidth
	...
	Private Sub agdDataMapping_RowColChange(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles agdDataMapping.RowColChange
		...
		With agdDataMapping
			...
			If .row <> lastrow Or .Col <> lastcol Then
				...
				SetSelFromGrid(.row)
			End If
			If .row > nRequiredFields Or .Col <> ColMappingName Then .set_ColEditable(.Col, True)
			.ClearValues()
		End With
	...
	Private Function FixedColLeft(ByRef index As Integer) As Integer
		FixedColLeft = ReadIntLeaveRest(Trim(agdDataMapping.get_TextMatrix(index, ColMappingCol)))
	End Function
	...
	Private Function FixedColRight(ByRef index As Integer) As Integer
		...
		Dim pos As Integer
		str_Renamed = Trim(agdDataMapping.get_TextMatrix(index, ColMappingCol))
	...
	Public Sub SelectFieldsDone()
		...
		'
		m_Prompt = " To map " & agdDataMapping.get_TextMatrix(1, ColMappingName) & " , click or enter a column number"
	...
	'
	Private Sub agdSample_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles agdSample.ClickEvent
		Dim m_Prompt As String
		Dim m_LenPro As Short
		'
		...
		If agdSample.SelStartCol = agdSample.SelEndCol And agdSample.SelStartRow = 1 And agdSample.SelEndRow = agdSample.rows Then
			...
			agdDataMapping.set_TextMatrix(agdDataMapping.row, ColMappingCol, agdSample.SelStartCol + 1)
	...
	Private Sub InitDataMapping()
		...
		With agdDataMapping
			...
			.set_ColTitle(ColMappingName, "Name")
			.set_ColTitle(ColMappingAttr, "Attribute")
			.set_ColTitle(ColMappingCol, "Input Column")
			.set_ColTitle(ColMappingConstant, "Constant")
			.set_ColTitle(ColMappingSkip, "Skip Values")
			.Col = ColMappingName
			For r = 1 To nRequiredFields
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object RequiredFields(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				.Ctlset_Text(RequiredFields(r))
				.CellBackColor = System.Drawing.SystemColors.Menu
				Select Case RequiredFields(r)
					Case "Year" : .set_TextMatrix(r, ColMappingConstant, "1900")
					Case "Day" : .set_TextMatrix(r, ColMappingConstant, "1")
					Case "Hour", "Minute", "Second" : .set_TextMatrix(r, ColMappingConstant, "0")
						
					Case "Scenario", "Location", "Constituent", "Description"
						.set_TextMatrix(r, ColMappingAttr, "yes")
					Case Else : .set_TextMatrix(r, ColMappingAttr, "no")
				End Select
			Next r
		End With
	End Sub
...
ParseFixedDef: 
			...
			r = RowNamed(ColName)
			With agdDataMapping
				.set_TextMatrix(r, ColMappingName, ColName)
				.set_TextMatrix(r, ColMappingCol, ColCol)
				.set_TextMatrix(r, ColMappingConstant, "")
			End With
	...
	Private Function RowNamed(ByRef FieldName As String) As Integer
		...
		With agdDataMapping
			...
			For r = 1 To maxRow
				If Trim(LCase(.get_TextMatrix(r, ColMappingName))) = srchName Then Exit For
			Next r
			If r > maxRow Then
				If Trim(.get_TextMatrix(maxRow, ColMappingName)) = "" Then r = maxRow
			End If
		End With
		RowNamed = r
	End Function
	...
	Private Sub SetDatePortion(ByRef scr As clsATCscriptExpression, ByRef subexpName As String)
		...
		If r <= agdDataMapping.rows Then
			...
			If LCase(scp) <> LCase(subexpName) Then
				agdDataMapping.set_TextMatrix(r, ColMappingConstant, scp)
			End If
		End If
	End Sub
	...
	Public Sub SetWizardFromScript(ByRef scr As clsATCscriptExpression)
		...
		Select Case scr.Token
			...
			Case clsATCscriptExpression.ATCsToken.tok_Attribute
				...
				agdDataMapping.set_TextMatrix(r, ColMappingName, str_Renamed)
				agdDataMapping.set_TextMatrix(r, ColMappingAttr, "yes")
				...
				agdDataMapping.set_TextMatrix(r, ColMappingConstant, str2)
			...
			Case clsATCscriptExpression.ATCsToken.tok_For
				If LCase(Trim(scr.SubExpression(1).Printable)) = "repeat" And scr.SubExpression(3).Token = clsATCscriptExpression.ATCsToken.tok_Literal Then
					r = RowNamed("Repeats")
					agdDataMapping.set_TextMatrix(r, ColMappingConstant, TrimQuotes(scr.SubExpression(3).Printable))
				End If
				ForMax = scr.SubExpressionCount
			...
			Case clsATCscriptExpression.ATCsToken.tok_In
				...
				If r <= agdDataMapping.rows Then
					...
					If ReverseLogic Then
						agdDataMapping.set_TextMatrix(r, ColMappingSkip, str_Renamed)
					End If
				End If
			...
			Case clsATCscriptExpression.ATCsToken.tok_NE
				...
				If r <= agdDataMapping.rows Then
					If Not ReverseLogic Then
						agdDataMapping.set_TextMatrix(r, ColMappingSkip, scr.SubExpression(2).Printable)
					End If
				End If
	...
	Private Function ScriptStringFromWizard() As String
		...
		For r = 1 To agdDataMapping.rows
			tmpstr = Trim(agdDataMapping.get_TextMatrix(r, ColMappingCol))
			If tmpstr <> "" Then
				tmpstr2 = Trim(agdDataMapping.get_TextMatrix(r, ColMappingName))
		...
		For r = 1 To agdDataMapping.rows
			tmpstr = LCase(Trim(agdDataMapping.get_TextMatrix(r, ColMappingAttr)))
			tmpstr2 = Trim(agdDataMapping.get_TextMatrix(r, ColMappingCol))
		...
		If Not SomeAttribVaries Then
			For r = 1 To agdDataMapping.rows
				tmpstr = Trim(agdDataMapping.get_TextMatrix(r, ColMappingConstant))
				If tmpstr <> "" Then
					tmpstr2 = Trim(agdDataMapping.get_TextMatrix(r, ColMappingCol))
					If tmpstr2 = "" Then
						If LCase(VB.Left(Trim(agdDataMapping.get_TextMatrix(r, ColMappingAttr)), 1)) = "y" Then
							str_Renamed = str_Renamed & PrintEOL & Space(indent) & "(Attribute "
						Else
							str_Renamed = str_Renamed & PrintEOL & Space(indent) & "(Set "
						End If
						str_Renamed = str_Renamed & Trim(agdDataMapping.get_TextMatrix(r, ColMappingName))
						str_Renamed = str_Renamed & " """ & tmpstr & """)"
					End If
				End If
			Next 
		...
		NestedIfs = 0
		For r = 1 To agdDataMapping.rows
			tmpstr = Trim(agdDataMapping.get_TextMatrix(r, ColMappingSkip))
			...
			For r = 1 To agdDataMapping.rows
				...
				If tmpstr <> "" Then
					...
					If commaPos = 0 Then
						...
						str_Renamed = str_Renamed & "(<> " & ConstOrCol(Trim(agdDataMapping.get_TextMatrix(r, ColMappingName))) & " " & tmpstr & ")"
					Else
						'UPGRADE_WARNING: Couldn't resolve default property of object ConstOrCol(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						str_Renamed = str_Renamed & "(Not (In " & ConstOrCol(Trim(agdDataMapping.get_TextMatrix(r, ColMappingName)))
						While commaPos > 0
...
SelectDataset: 
	...
	Private Function ConstOrCol(ByRef FieldName As String) As Object
		...
		Else
			...
			Else
				colStr = Trim(agdDataMapping.get_TextMatrix(r, ColMappingCol))
				If Len(colStr) > 0 Then colStr = FieldName
				retval = colStr
				constStr = Trim(agdDataMapping.get_TextMatrix(r, ColMappingConstant))
	...
	Private Sub PopulateGridSample()
		...
		If UnitDataFile > 0 Then
			...
			With agdSample
				...
				'      Next c
				'    End If
				While Not EOF_Renamed()(UnitDataFile) And linecnt < lines
					cbuff = LineInput(UnitDataFile)
					pcols = ParseInputLine(cbuff, parsed)
					linecnt = linecnt + 1
					For c = 1 To pcols
						.set_TextMatrix(linecnt, c - 1, parsed(c))
					Next c
				End While
				'If chkHeaderRecord.Value <> 1 Then 'number columns
				For c = 0 To .Cols - 1
					If delimQ Then .set_ColTitle(c, c + 1) Else .set_ColTitle(c, FixedColLeft(c + 1) & "-" & FixedColRight(c + 1))
				Next c
				'End If
				Seek(UnitDataFile, 1)
			End With
	...
	Private Sub PopulateTxtSample()
		...
		If LenDataFile > 0 Then
			...
			nChars = VB6.PixelsToTwipsX(txtSample(0).Width) / CharWidth - 1
			linecnt = 0
...
exitsub: 
	...
	Private Sub optDelimiter_CheckedChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles optDelimiter.CheckedChanged
		If eventSender.Checked Then
			...
			If delimQ Then
				...
				agdDataMapping.set_ColTitle(ColMappingCol, "Input Column")
			Else
				...
				agdDataMapping.set_ColTitle(ColMappingCol, "Beg-End Column")
			End If
			PopulateSample()
		End If
	End Sub
	...
	Private Sub txtSampleAnyChange(ByRef whichChanged As Short)
		...
		If SelLength > 0 And Not SettingSelFromGrid Then
			If SelLength < 2 Then
				agdDataMapping.set_TextMatrix(agdDataMapping.row, ColMappingCol, SelStart)
			Else
				agdDataMapping.set_TextMatrix(agdDataMapping.row, ColMappingCol, SelStart & "-" & SelStart + SelLength - 1)
			End If
	...
	Private Sub cmdBrowseData_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdBrowseData.Click
		...
		If UnitDataFile > 0 Then FileClose(UnitDataFile)
		'UPGRADE_WARNING: Filter has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
	...
	Private Sub AskForLookupFilename()
		...
		dlgOpenFileSave.FileName = dlgOpenFileOpen.FileName
		With agdDataMapping
			.set_TextMatrix(.row, 0, dlgOpenFileOpen.FileName)
			If Len(dlgOpenFileOpen.FileName) > 0 Then .set_TextMatrix(.row, 4, "")
		End With
	End Sub
