...
Begin VB.Form frmShapeAttributes 
   ...
   ClientTop       =   285
   ClientWidth     =   4560
   Icon            =   "frmShapeAttributes.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   4725
   ScaleWidth      =   4560
...
Private Sub cmdOk_Click()
  Dim i&, j&
  SaveSetting App.EXEName, "ShapeAttributes", "X", cboX.Text
  SaveSetting App.EXEName, "ShapeAttributes", "Y", cboY.Text
  i = 0
  For j = 0 To lstAttributes.ListCount - 1
    If lstAttributes.Selected(j) Then
      i = i + 1
      SaveSetting App.EXEName, "ShapeAttributes", "last" & i, lstAttributes.List(i)
    End If
  Next
...
Public Sub SetDataFile(tsfile As ATCclsTserFile, ByVal PreSelectedAttributes As String)
...
'  Dim lastSelected As New FastCollection
  ...
  lastX = GetSetting(App.EXEName, "ShapeAttributes", "X", "LNGDEG")
  lastY = GetSetting(App.EXEName, "ShapeAttributes", "Y", "LATDEG")
  If Len(PreSelectedAttributes) = 0 Then
    PreSelectedAttributes = GetSetting(App.EXEName, "ShapeAttributes", "Selected")
  End If
  If Len(PreSelectedAttributes) > 0 Then
...
'  Wend
  ...
  For i = 1 To AllAttributes.Count
    Attrib = AllAttributes(i).Name
    cboX.AddItem Attrib: If Attrib = lastX Then cboX.ListIndex = i - 1
    cboY.AddItem Attrib: If Attrib = lastY Then cboY.ListIndex = i - 1
    lstAttributes.AddItem Attrib
    If Len(PreSelectedAttributes) > 0 Then
      If InStr(PreSelectedAttributes, "," & Attrib & ",") > 0 Then lstAttributes.Selected(i - 1) = True
    Else
      If Len(tsfile.Data(1).Attrib(Attrib)) > 0 Then lstAttributes.Selected(i - 1) = True
    End If
'    If lastSelected.KeyExists(Attrib) Then lstAttributes.Selected(i - 1) = True
  Next
End Sub
