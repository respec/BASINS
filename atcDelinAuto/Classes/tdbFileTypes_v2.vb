...
Public Class tdbFileTypes_v2
    ...
    Public Sub formFileNames(ByVal demfn As String, ByVal outFolder As String, ByVal checkDirExists As Boolean)
        dem = demfn
        Dim strPathBgd, tmpFile As String
        Dim outPath As String
        Dim g As New MapWinGIS.Grid
        Dim absoutdir As String
        If IO.Path.GetFileName(demfn) = "sta.adf" Then
            absoutdir = getAbsolutePath(outFolder, IO.Path.GetDirectoryName(demfn) + ".bgd")
        Else
        ...
        '
        dem = demfn
        If System.IO.Path.GetExtension(dem) <> ".bgd" Then
            If System.IO.Path.GetFileName(dem) = "sta.adf" Then
                tmpFile = System.IO.Path.GetDirectoryName(dem) + ".tmp"
                strPathBgd = getAbsolutePath(tdbChoiceList.OutputPath, tmpFile) + IO.Path.GetFileNameWithoutExtension(tmpFile) + ".bgd"
            Else
                strPathBgd = getAbsolutePath(tdbChoiceList.OutputPath, demfn) + IO.Path.GetFileNameWithoutExtension(demfn) + ".bgd"
            End If
            g.Open(dem)
            g.Save(strPathBgd)
            g.Close()
