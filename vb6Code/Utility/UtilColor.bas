...
Private colorDatabase As clsDBF
...
Public Sub InitMatchingColors(Filename As String)
  ...
  If FileExists(Filename) Then MatchingColorsFilename = Filename
...
Private Function colorDB() As clsDBF
  ...
  If Not openedDB Then
    ...
    If FileExists(DBpath) Then
      Set colorDatabase = New clsDBF
      colorDatabase.OpenDBF DBpath
      openedDB = True
    End If
  End If
...
Public Sub testColor(fwd As Boolean, _
  prevColorName As String, _
  ByRef nextColorName As String, _
  ByRef nextColor As OLE_COLOR)
...
Public Function TextOrNumericColor(ByVal colr As String) As Long
...
Public Function colorName(ByVal rgb As Long) As String
  ...
  If db.FindFirst(2, CStr(rgb)) Then
    retval = db.Value(1)
  Else
