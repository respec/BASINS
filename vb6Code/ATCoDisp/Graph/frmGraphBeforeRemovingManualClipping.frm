...
Private Sub DrwLines()
  ...
  Dim xaxtyp As Long
  'Dim clipfg As Long
  'Dim bvflg As Long  'bad value flag (outside current graph)
  ...
  'Dim pts() As POINTAPI, hndBr As Long, oldBr As Long
On Error GoTo errorhandler10
  'SetLogFont Graph.Font
  ...
  For i = 0 To ncrv - 1
    ...
    If ymax <> ymin Then
      ...
      'LineTo Graph.hdc, ClipX1, ClipY1
      'LineTo Graph.hdc, ClipX2, ClipY2
      'LineTo Graph.hdc, ClipX2, ClipY1
      'LineTo Graph.hdc, ClipX1, ClipY2
      ...
      pos = 0
      'pts(pos).x = Wc2DcX(Graph, 0)
      'pts(pos).y = Wc2DcY(Graph, 0)
      pos = pos + 1
      For ypos = minypos To maxypos
        ...
        If Var(ky).Trans = 1 Then 'arithmetic Y
          ...
          'pts(pos).y = Wc2DcY(Graph, CSng(yx(ypos)))
        Else 'logarithmic Y
          If yx(ypos) <> 0 Then
          ...
          End If
          'pts(pos).y = Wc2DcY(Graph, Log10(CDbl(yx(ypos))))
        End If
'        bvflg = 0
'        If xp < 0 Then 'value off plot to left
'          bvflg = 1
'        ElseIf xp > XLen Then 'value off right
'          bvflg = 2
'        ElseIf yp > yylen + base Then 'value off top
'          bvflg = 3
'        ElseIf yp < base Then 'value off bottom
'          bvflg = 4
'        End If
'        If bvflg = 0 Then 'good point
          If Crv(i).LType >= 0 Then 'plot lines
'            If clipfg = 1 Then
'              'previous point was bad,
'              'put additional point at edge of axis
'              If meanfg Then
'                slope = 0
'              Else
'                slope = (ytmp - yp) / (xtmp - xp)
'              End If
'              If xtmp < 0 Then 'off left side
'                Graph.CurrentX = 0
'                Graph.CurrentY = ytmp + (0 - xtmp) * slope
'              ElseIf xtmp > XLen Then 'off right side
'                Graph.CurrentX = XLen
'                Graph.CurrentY = ytmp + (XLen - xtmp) * slope
'              ElseIf ytmp >= yylen + base Then 'off top
'                If meanfg Then
'                  'xtmp = xp - xinc
'                  Graph.CurrentX = xtmp
'                Else
'                  Graph.CurrentX = xtmp + (yylen - ytmp) / slope
'                End If
'                Graph.CurrentY = yylen + base
'              ElseIf ytmp <= base Then 'off bottom
'                If meanfg Then
'                  'xtmp = xp - xinc
'                  Graph.CurrentX = xtmp
'                Else
'                  Graph.CurrentX = xtmp + (0 - ytmp) / slope
'                End If
'                Graph.CurrentY = base
'              End If
'              clipfg = 0
'            End If
          ...
          If DataLabelPosition > 0 And nDataLabels > 0 Then
            ...
            End If
          End If
'        Else 'value off axes, clip it
'          If Crv(i).LType >= 0 And ypos > minypos Then
'            'find location at edge of plot
'            If meanfg Then
'              slope = 0
'            Else
'              slope = (yp - ytmp) / (xp - xtmp)
'            End If
'            If xp < 0 Then 'off left side
'              xtmp = 0
'              ytmp = ytmp + (0 - xtmp) * slope
'            ElseIf xp > XLen Then 'off right side
'              xtmp = XLen
'              ytmp = ytmp + (XLen - xtmp) * slope
'            ElseIf yp > yylen + base Then 'off top
'              If meanfg Then
'                'xp = xtmp
'              Else
'                xtmp = xtmp + (yylen - ytmp) / slope
'              End If
'              ytmp = yylen + base
'            ElseIf yp < base Then 'off bottom
'              If meanfg Then
'                'xp = xtmp
'              Else
'                xtmp = xtmp + (0 - ytmp) / slope
'              End If
'              ytmp = base
'            End If
'            If clipfg = 0 Then 'previous point was good
'              Graph.Line -(xtmp, ytmp), clr
'            End If
'          End If
'          clipfg = 1
'        End If
      ...
      'If fill(i) > 0 Then
      '  pts(pos).x = Wc2DcX(Graph, XLen)
      '  pts(pos).y = Wc2DcY(Graph, YLen)
      '  pos = pos + 1
      '  hndBr = CreateSolidBrush(QBColor(i + 1))
      '  oldBr = SelectObject(Graph.hdc, hndBr)
      '  j = Polygon(Graph.hdc, pts(0), pos)
      '  j = SelectObject(Graph.hdc, oldBr)
      '  j = DeleteObject(hndBr)
      'End If
    End If
  ...
  Next i
  DrwMathLines
  'DrwConstY ymin, ymax, yinc
  'DrwConstX xmin, xmax, xinc, yylen, base
