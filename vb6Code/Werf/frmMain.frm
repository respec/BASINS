...
Private Sub Form_Load()
  ...
  If Screen.Width / Screen.TwipsPerPixelX < 1024 Then
    MsgBox "SYSTEM NOTE: A Screen Resolution of 1024x768 is required to use this product most effectively."
  End If
End Sub
...
Private Sub Form_Resize()
  ...
  If w > 4000 And h > 4000 Then
    ...
    For i = 0 To fraOpt.Count - 1
      fraOpt(i).Width = fraCriteria.Width
    Next
...
Private Sub optValue_Click(Index As Integer)
  If chkValue(CInt(optValue(Index).Tag)).Value = vbChecked Then
    optValue(Index).Value = True
  Else
    optValue(Index).Value = False
  End If
End Sub

Private Sub optValue_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
  If chkValue(CInt(optValue(Index).Tag)).Value = vbChecked Then
    optValue(Index).Value = False
    chkValue(CInt(optValue(Index).Tag)).Value = vbUnchecked
  Else
    chkValue(CInt(optValue(Index).Tag)).Value = vbChecked
  End If
End Sub
...
Private Function ExclusiveChoice(CriteriaName As String) As Boolean
  Select Case CriteriaName
    Case "Level of Effort", _
         "Data Requirements", _
         "Modeler Expertise", _
         "Model Availability": ExclusiveChoice = True
    Case Else:                 ExclusiveChoice = False
  End Select
End Function
...
Private Sub RefreshCriteria(t$)
  ...
  Dim MakingRadioButtons As Boolean, nOpt As Long
  Dim fra As Frame
  ...
  Wend
  chkValue(0) = False
  While optValue.Count > 1
    Unload optValue(optValue.Count - 1)
  Wend
  While fraOpt.Count > 1
    Unload fraOpt(fraOpt.Count - 1)
  Wend
  fraOpt(0).Visible = False
  ...
  While Not myCriteria.EOF
    If t = Left(myCriteria!Type, 4) Then
      ...
      lKcnt = 0
      myValue.MoveFirst
      MakingRadioButtons = ExclusiveChoice(myCriteria!Name)
      If MakingRadioButtons Then
        If fraOpt(0).Visible Then Load fraOpt(fraOpt.Count)
        Set fra = fraOpt(fraOpt.Count - 1)
        fra.Width = fraCriteria.Width
        fra.Top = lTop
        fra.Visible = True
      End If
      While Not myValue.EOF
        If t = Left(myValue!Type, 4) And myCriteria!Name = myValue!criteria Then
          ...
          If MakingRadioButtons Then
            If nOpt > 0 Then Load optValue(nOpt)
            optValue(nOpt).Caption = s
            optValue(nOpt).Width = wid
            optValue(nOpt).Tag = lVcnt
            optValue(nOpt).Value = False
            optValue(nOpt).Visible = True
            Set optValue(nOpt).Container = fra
            nOpt = nOpt + 1
          End If
        ...
        myValue.MoveNext
      Wend
      If MakingRadioButtons Then fra.ZOrder        'Move to front to cover checkboxes
  ...
  For ResizeRow = 0 To lCcnt - 1
    ...
    End If
  Next
  For ResizeIndex = 0 To nOpt - 1
    optValue(ResizeIndex).Left = chkValue(optValue(ResizeIndex).Tag).Left
  Next
