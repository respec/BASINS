...
'reads ISH stations starting with 99 for the year 2005 (left off original processing)
Public Module ReadISHUpdates200599
    ...
    Public Sub ScriptMain(ByRef aMapWin As IMapWin)
        Logger.Dbg("ReadISHUpdates-2005-99:Start")
        ChDriveDir(pInputPath)
        Logger.Dbg(" CurDir:" & CurDir())

        Dim lNewYears() As String = {"2005"}
        ...
        If lISHDBF.OpenFile(lStaHistory) Then
            Logger.Dbg("ReadISHUpdates-2005-99: Opened Station History file " & lStaHistory)
        Else
            Logger.Dbg("ReadISHUpdates-2005-99: Problem opening Station History file " & lStaHistory)
        End If
        ...
        Logger.Dbg("ReadISHUpdates-2005-99: Updating ISH data from directory " & pInputPath)
        For Each lYear As String In lNewYears
            lCnt = 0
            Dim lFiles As New NameValueCollection
            Logger.Dbg(" ")
            Logger.Dbg("ReadISHUpdates-2005-99: Updating for Year " & lYear)
            AddFilesInDir(lFiles, pInputPath & lYear & "\", True)
            Logger.Dbg("ReadISHUpdates-2005-99: Found " & lFiles.Count & " update files")

            For Each lFile As String In lFiles
                Logger.Dbg("ReadISHUpdates-2005-99: Opening data file - " & lFile)
                lStaID = FilenameNoPath(lFile).Substring(0, 6)
                If lStaid.StartsWith("99") Then
                    If lISHDBF.FindFirst(1, lStaID) Then
                        ...
                        If lState <> "" Then
                            Logger.Dbg("ReadISHUpdates-2005-99: " & lStaID & " was found in history file")
                            lStaName = lISHDBF.Value(3)
                            lLat = lISHDBF.Value(11)
                            lLng = lISHDBF.Value(12)
                            Logger.Dbg("ReadISHUpdates-2005-99: Found " & lNOAAFile.DataSets.Count & " data sets")
                            If lNOAAFile.DataSets.Count > 0 Then
                                ...
                                If lOutFile.Count = 1 Then 'WDM file exists, copy to working version
                                    Logger.Dbg("ReadISHUpdates-2005-99: Found existing WDM file for station " & lStaID)
                                    FileCopy(lOutFile.Item(0), lCurWDM)
                                    If Right(PathNameOnly(lOutFile.Item(0)), 2).ToUpper <> lState.ToUpper Then
                                        Logger.Dbg("ReadISHUpdates-2005-99: NOTE - Existing data not in same state as current data")
                                        Logger.Dbg("ReadISHUpdates-2005-99:        *** Removed existing WDM file " & lOutFile.Item(0))
                                        Kill(lOutFile.Item(0))
                                    End If
                                ElseIf lOutFile.Count > 1 Then
                                    Logger.Dbg("ReadISHUpdates-2005-99: PROBLEM!!!  Found " & lOutFile.Count & " WDM files for station " & lStaID)
                                ElseIf lOutFile.Count = 0 Then 'new station
                                    Logger.Dbg("ReadISHUpdates-2005-99: Creating new WDM file for station - " & lStaID)
                                End If
                                Dim lWDMfile As New atcWDM.atcDataSourceWDM
                                lWDMfile.Open(lCurWDM)
                                'Logger.Dbg("ReadISHUpdates-2: Opening WDM file - " & lWDMfile.Name)
                                lWriteProblem = False
                                For Each lDS As atcDataSet In lNOAAFile.DataSets
                                    ...
                                    For Each lWDMDS In lWDMfile.DataSets
                                        If lDS.Attributes.GetValue("Constituent") = lWDMDS.Attributes.GetValue("Constituent") Then
                                            ...
                                            'Logger.Dbg("ReadISHUpdates-2: Matched Cons " & lTS.Attributes.GetValue("Constituent") & " End of existing: " & lTS.Dates.Value(lTS.numValues))
                                            lUpdating = True
                                            Exit For
                                        End If
                                    Next
                                    If lUpdating Then
                                        If lWDMfile.AddDataset(lDS, atcDataSource.EnumExistAction.ExistAppend) Then
                                            Logger.Dbg("ReadISHUpdates-2005-99: Updated " & lDS.Attributes.GetValue("Constituent") & " on DSN " & lDS.Attributes.GetValue("ID"))
                                        Else 'try merging existing and new datasets
                                            ...
                                            If lDSRevised Is Nothing Then
                                                lWriteProblem = True
                                                Logger.Dbg("ReadISHUpdates-2005-99: PROBLEM updating " & lDS.Attributes.GetValue("Constituent") & " on DSN " & lDS.Attributes.GetValue("ID") & " - Could not Merge")
                                            ElseIf lWDMfile.AddDataset(lDSRevised, atcDataSource.EnumExistAction.ExistReplace) Then
                                                Logger.Dbg("ReadISHUpdates-2005-99: Merged " & lDS.Attributes.GetValue("Constituent") & " on DSN " & lDS.Attributes.GetValue("ID"))
                                            Else
                                                lWriteProblem = True
                                                Logger.Dbg("ReadISHUpdates-2005-99: PROBLEM updating " & lDS.Attributes.GetValue("Constituent") & " on DSN " & lDS.Attributes.GetValue("ID") & " - still failed after Merge")
                                            End If
                                            lDSRevised = Nothing
                                        End If
                                    Else 'new dataset, save station attributes
                                        ...
                                        If lWDMfile.AddDataset(lDS, atcDataSource.EnumExistAction.ExistRenumber) Then
                                            Logger.Dbg("ReadISHUpdates-2005-99: NEW data for " & lDS.Attributes.GetValue("Constituent") & " on DSN " & lDS.Attributes.GetValue("ID"))
                                        Else
                                            Logger.Dbg("ReadISHUpdates-2005-99: PROBLEM writing new data for " & lDS.Attributes.GetValue("Constituent") & " on DSN " & lDS.Attributes.GetValue("ID"))
                                        End If
                                    End If
                                Next
                                ...
                                If lWriteProblem Then 'problem, keep existing WDM file, write new with different name
                                    ...
                                    Logger.Dbg("ReadISHUpdates-2005-99: PROBLEM updating station " & lStaID & " - update saved to " & lOutWDM)
                                    FileCopy(lCurWDM, lOutWDM)
                                Else 'everything looks good, remove existing WDM file, then copy to it
                                    If FileExists(lOutWDM) Then
                                        Kill(lOutWDM)
                                        Logger.Dbg("ReadISHUpdates-2005-99: Updated " & lOutWDM)
                                    Else
                                        Logger.Dbg("ReadISHUpdates-2005-99: Created " & lOutWDM)
                                    End If
                                    FileCopy(lCurWDM, lOutWDM)
                                End If
                            ...
                            End If
                        Else
                            Logger.Dbg("ReadISHUpdates-2005-99: Skipping station " & lStaID & " - not in a valid State")
                        End If
                        lNOAAFile.DataSets.Clear()
                        lNOAAFile = Nothing
                    Else
                        Logger.Dbg("ReadISHUpdates-2005-99: Skipping station " & lStaID & " - not found in history file")
                    End If
                End If
            Next
            Logger.Dbg("ReadISHUpdates-2005-99: Completed Year " & lYear & " - processed " & lCnt & " stations")
        Next
        Logger.Dbg("ReadISHUpdates-2005-99: Completed Updating of ISH data")
