...
'             and expanded. If all files are to be extracted, then this can be NULL.
...
Private Declare Sub Wiz_NoPrinting Lib "unzip32.dll" (ByVal flag As Long)

Private Declare Function Wiz_Validate Lib "unzip32.dll" _
  (ByVal archive As String, _
   ByVal AllCodes As Long) As Long
Private Declare Function Wiz_UnzipToMemory Lib "unzip32.dll" _
  (ByVal archive As String, _
   ByVal file As String, _
   Userf As USERFUNCTION, _
   buf As UzpBuffer) As Long
...
'    retcode = ZpInit(MYUSER)
  ...
  MYOPT.fPrivilege = 0     ' 1 if not saving privileges
...
Public Function VBUnzipVersion() As UZPVER
  Dim UZVER As UZPVER
  With UZVER
    .structlen = Len(UZVER)
    .beta = Space$(9) & vbNullChar
  ...
  End With
  UzpVersion2 UZVER
  VBUnzipVersion = UZVER
End Function

Public Function VBUnzipToBuffer(ByRef sZipFileName As String, _
                                ByRef sFilename As String, _
                                ByRef sBuffer As String) As Long

On Error GoTo vbErrorHandler
  Dim i As Long
  Dim lRet As Long
  Dim UZUSER As USERFUNCTION
  Dim UZBuffer As UzpBuffer
  ...
  UZBuffer.buf = String(1000, "-")
  lRet = Wiz_UnzipToMemory(sZipFileName, sFilename, UZUSER, UZBuffer)
  VBUnzipToBuffer = lRet
  sBuffer = "" 'UZBuffer.buf
  For i = 1 To UZBuffer.length
    sBuffer = sBuffer & Mid(UZBuffer.buf, i, 1)
  Next
  'sBuffer = Left(UZBuffer.buf, UZBuffer.strlength)
  Exit Function
