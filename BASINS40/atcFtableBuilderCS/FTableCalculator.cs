...
namespace atcFtableBuilder
{
    ...
    public abstract class FTableCalculator
    {
        ...
        public virtual ArrayList GenerateFTableOC()
        {
            return null;
        }
 ...
 */
        ...
        /// <summary>
        /// new routine for merging or matching channel flow to all control device outflow,
        /// organize by exits
        /// </summary>
        /// <param name="openChannelVector"></param>
        /// <param name="controlDeviceVectors">a ExitId <-> List_of_CD_Outflow structure</param>
        /// <returns>a ExitID <-> Outflow_table structure, all exits' outflow tables have the same first 4 columns from the channel flow table </returns>
        public ArrayList mergeCalculatorsME(ArrayList openChannelVector, Dictionary<int, Dictionary<string, ArrayList>> controlDeviceVectors)
        {
            double lEpslon = 0.0000001;
            ArrayList gridVector = new ArrayList();
            ArrayList channelFlowVector;
            double currentDepth = 0;
            double lastFlowReading = 0;
            hasDepthEverBeenFound = false;
            //FTableCalculatorConstants.DepthCheckFinal=-1;
            ...
            for (int i = 0; i < openChannelVector.Count; i++)
            {
                channelFlowVector = (ArrayList)openChannelVector[i];

                //get the depth on that row
                double.TryParse(channelFlowVector[0].ToString(), out currentDepth);

                //go over each selected control device and find out if its depth is > the depth of the stream
                //for (Enumeration e = controlDeviceVectors.keys();e.hasMoreElements();)

                // FTableCalculatorConstants.DepthCheckFinal=0;
                double lTotalCDOutflow = 0;
                for (int e = 2; e <= 5; e++)
                {
                    lTotalCDOutflow = 0;
                    //ArrayList lChannelFlowCopy = (ArrayList)openChannelVector.Clone();
                    ...
                    foreach (string lCDTypeKey in lCDOutflows.Keys)
                    {
                        ArrayList cdGridVector = (ArrayList)lCDOutflows[lCDTypeKey];
                        bool foundDepth = false;
                        //FTableCalculatorConstants.DepthCheck=0; // sri-08-27-2012
                        for (int j = 0; j < cdGridVector.Count; j++)
                        {
                            ...
                            if (Math.Abs(controlDeviceDepth - currentDepth) < lEpslon)
                            {
                                // FTableCalculatorConstants.DepthCheck=1;  // sri-08-27-2012
                                foundDepth = true;
                                double laCDOutflowVal = 0;
                                double.TryParse(cdRowVector[3].ToString(), out laCDOutflowVal);
                                lTotalCDOutflow += laCDOutflowVal;

                                //channelFlowVector.Add(laCDOutflowVal);
                                lastFlowReading = laCDOutflowVal;
                                hasDepthEverBeenFound = true;
                                //since you found the depth, go to the next control device
                                break;
                            }
                    ...
                    //if (gridVector.Count == 0)
                    //    lMergedOutputs.Add(e, null);
                    //else
                    //    lMergedOutputs.Add(e, gridVector);
                }

                gridVector.Add(channelFlowVector);
            }
            return gridVector;
        }
