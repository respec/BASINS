...
Public Class atcHydroligicIndices
  ...
  Public Overrides ReadOnly Property Description() As String
    Get
      ...
      retval = Left(retval, Len(retval) - 2) & ";" 'Replace last ", " with ";"
    ...
    For Each arg As Object In args
      If retval.GetType.IsAssignableFrom(arg.GetType) Then 'atcTimeseries
        ...
        If curTS.numValues <> firstTS.numValues Then
          Err.Raise("Different lengths of atcTimeseries passed to " & Name & ".ComputeTimeseries")
        Else
          tsArgs.Add(curTS)
        End If
      Else 'not atcTimeseries, probably a constant
        If numberArg <> 0 Then
          Err.Raise("More than one constant passed to " & Name & ".ComputeTimeseries not yet supported")
        End If
        numberArg = CDbl(arg)
      End If
    Next
