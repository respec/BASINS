Attribute VB_Name = "modUCIRecords"
Option Explicit

Dim uciRec() As String
Dim uciRecCnt As Long

Public Sub ReadUCIRecords(pName As String)
  Dim i&, s$
...
Public Sub GetUCIRecord(i As Long, s As String)
  s = uciRec(i)
End Sub

Public Sub GetNextRecordFromBlock(blockname$, retkey&, cbuff$, rectyp&, retcod&)
  Dim i&, ilen&
...
Public Sub StartingRecordofOperationTable(opname$, kwd$, srec&, noccur&)
  Dim i&, ilen&, ostart&, oend&
...
Public Sub GetNextRecordFromTable(blockname$, tablename$, srec&, initfg&, noccur&, cbuff$, rectyp&, retcod&)
  Dim i&, ilen&
...
Public Sub GetCommentBeforeBlock(blockname$, Comment$)
  Dim i&, ilen&, retkey&
...
Public Sub GetTableComment(srec&, tabname$, thisoccur&, Comment$)
  Dim i&, ilen&, retkey&, noccur&
...
Public Sub SaveBlockOrder(cOrder As Collection)
  'default order is:
  '"GLOBAL"
  '"FILES"
  '"OPN SEQUENCE"
  ...
  For i = 1 To cOrder.Count
    ...
    If Not bfound Then
      ...
      If iprev = 0 Then
        myOrder.Add cOrder(i), , 1
      Else
        'figure out where to put it
        myOrder.Add cOrder(i), , , iprev
        iprev = iprev + 1
      End If
    End If
  Next i
