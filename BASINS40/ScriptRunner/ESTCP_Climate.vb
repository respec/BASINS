...
Public Module ESTCP_Climate
    ...
    Private Sub UpdateFBLandUse()
        Dim lMsg As New atcUCI.HspfMsg
        lMsg.Open("hspfmsg.wdm")

        Dim lOrigUCI As New HspfUci
        lOrigUCI.FastReadUciForStarter(lMsg, "D:\ESTCP\FB\Model_EB\Upatoi.uci")

        Dim lPerMultUpper As Double = 0.0189
        Dim lImpMultUpper As Double = 0.0312
        Dim lPerMultLower As Double = 0.0404
        Dim lImpMultLower As Double = 0.0578
            ...
            For Each lConn As HspfConnection In lOrigUCI.Connections
                If lConn.Target.VolName = "RCHRES" And lConn.Target.VolId = lRchOper.Id Then
                    ...
                    If lConn.Source.VolName = "PERLND" Then
                        Dim lPerOpn As HspfOperation = lOrigUCI.OpnBlks("PERLND").OperFromID(lConn.Source.VolId)
                        If lPerOpn.Description.StartsWith("U/C") Or lPerOpn.Description.StartsWith("Paved Road") Or lPerOpn.Description.StartsWith("Water") Then
                        Else
                            lPerOrigArea += lConn.MFact
                        End If
                    End If
