...
Public Module StorePRISM
    Private Const pInputPath As String = "C:\BASINS\Data\met_data\CA\"
    Private Const pPRISMPath As String = "C:\BASINS\Data\met_data\CA\"
    ...
    Public Sub ScriptMain(ByRef aMapWin As IMapWin)
        ...
        Dim lCurWDM As String = "Precip_12-8-06.wdm"
        ...
        If lPrismPrecip.OpenFile(pPRISMPath & "CA_ppt_ann.dbf") Then
            Logger.Dbg("StorePRISMCA: Opened PRISM Precip file " & lPrismPrecip.FileName)
        End If
        ...
        For Each lDS As atcDataSet In lDataSets
            ...
            lStation = lDS.Attributes.GetValue("Location")
            Logger.Dbg("StorePRISMCA: For Station - " & lStation)
            Select Case lCons
                Case "PRCP", "PREC", "HPCP", "HPCP1"
                    If lPrismPrecip.FindFirst(1, lStation) Then
                        ...
                        If lVal > 0 AndAlso lVal < 200 Then
                            ...
                            lRewrite = True
                        Else
                            Logger.Dbg("StorePRISMCA:  NOTE - No valid value found on PRISM Precip DBF file - no attribute stored")
                        End If
                    Else
                        Logger.Dbg("StorePRISMCA:  PROBLEM - could not find station " & lStation & " on PRISM Precip DBF file")
                    End If
            ...
            If lRewrite Then
                If lWDMfile.AddDataset(lDS, atcDataSource.EnumExistAction.ExistReplace) Then
                    Logger.Dbg("StorePRISMCA: Updated timeseries " & lDS.ToString & " with PRISM value of " & lVal)
                Else
                    Logger.Dbg("StorePRISMCA: PROBLEM Updating timeseries " & lDS.ToString & " with PRISM value")
                End If
            Else
                Logger.Dbg("StorePRISMCA: No PRISM data available for timeseries " & lDS.ToString)
            End If
        Next
        ...
        Logger.Dbg("StorePRISMCA:Completed PRISM Storing")
