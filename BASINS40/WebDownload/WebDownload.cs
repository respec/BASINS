...
/// This source code was originally part of NASA WorldWind
/// http://sourceforge.net/projects/nasa-exp/files/World Wind .NET/1.4/
/// Extended to work with FTP sites by Mark Gray of AQUA TERRA CONSULTANTS
...
namespace WorldWind.Net
{
	...
	public class WebDownload : IDisposable
	{
		...
		protected void Download()
		{
			...
			// http and ftp are both supported now so don't: Debug.Assert(Url.StartsWith("http://"));
			try
            {
                ...
                do
                {
                    ...
                    try
                    {
					    ...
					    bool isFTP = (request.GetType().Name == "FtpWebRequest");

                        if (!isFTP) // FTP does not have UserAgent
                        {
                        ...
                        using (response = request.GetResponse() as WebResponse)
                        {
                            if (isFTP || (((HttpWebResponse)response).StatusCode == HttpStatusCode.OK))
                            {
                                if (!isFTP)
                                {
                                    ContentType = response.ContentType;
                                    ContentEncoding = ((System.Net.HttpWebResponse)response).ContentEncoding;
                                }

                                // Find the data size from the appropriate property of the response
                    ...
                    catch (Exception caught)
                    {
                        try
                        {
                            // Remove broken file download if incomplete
                            if (ContentStream != null)
                            {
                                if (ContentLength > 0 && ContentStream.Length == ContentLength)
                                {   // Download was complete before exception, so keep it
                                    IsComplete = true;
                                }
                            ...
                            if (!IsComplete && SavedFilePath != null && SavedFilePath.Length > 0)
                            {
                                File.Delete(SavedFilePath);
                            }
                            if (caught.Message.StartsWith("Cannot handle redirect")) //for example when redirect goes from http: to ftp:
                            {
                                redirectURL = ((WebException)caught).Response.Headers.Get("Location");
                            }
                        }
                        catch (Exception)
                        {
                        }
                        if (!IsComplete && redirectURL == null)
                        {
